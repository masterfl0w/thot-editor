...
function cloneWorkspaceSnapshot(snapshot: WorkspaceSnapshot): WorkspaceSnapshot {
  return {
    ...
    texts: Object.fromEntries(
      Object.entries(snapshot.texts).map(([id, text]) => [id, { ...text }]),
    ),
...
function getWorkspaceSnapshot(
  state: Pick<DiagramState, 'nodes' | 'texts' | 'edges' | 'viewport' | 'zoom'>,
): WorkspaceSnapshot {
      ...
      ...(() => {
        const applyWorkspaceChange = (
          ...
          set((state) => ({
            ...
            historyFuture: [],
            actionHistory: [
              ...state.actionHistory.slice(-(MAX_ACTION_HISTORY - 1)),
              makeHistoryEntry(action, state.collaboration),
            ],
          }))
        }
        ...
        return {
          ...
          addBox: (opts = {}) => {
            ...
            if (opts.parent && nodes[opts.parent]) {
              ...
              const parent = {
                ...nodes[opts.parent],
                children: [...nodes[opts.parent].children, id],
              }
          ...
          deleteEdge: (idx) => {
            const { edges } = get()
            applyWorkspaceChange('Delete link', {
          ...
          updateText: (id, patch) => {
            const { texts } = get()
            if (!texts[id]) return
            applyWorkspaceChange('Edit text', {
          ...
          selectNode: (id) => {
            set({
              ...
              selEdge: null,
              multiSel: new Set(),
              modeText: 'Drag outside parent to extract · Drag onto another box to nest',
            })
          },
          ...
          selectText: (id) => {
            set({
              ...
              selEdge: null,
              multiSel: new Set(),
              modeText: 'Double-click text to edit inline',
            })
          },
          ...
          selectEdge: (idx) => {
            set({
              ...
              selText: null,
              multiSel: new Set(),
              modeText: 'Link selected — edit or Delete to remove',
            })
          },
          ...
          startConnect: (nodeId, side = 'pr') => {
            ...
            set({
              ...
              csrc: src,
              csrcSide: side,
              modeText: 'Drag from one port to another port · Esc to cancel',
            })
          },
          ...
          copySelectionToClipboard: () => {
            ...
            const nodeList = [...rootNodeIds].map((id) => ({
              ...nodes[id],
          ...
          pasteClipboard: (at = null) => {
            ...
            if (
              !sceneClipboard ||
              (sceneClipboard.nodes.length === 0 && sceneClipboard.texts.length === 0)
            )
              return false

            const includedParents = new Set(sceneClipboard.nodes.map((node) => node.id))
            const topLevelNodes = sceneClipboard.nodes.filter(
              (node) => !node.parent || !includedParents.has(node.parent),
            )
            const positioned = [
            ...
            sceneClipboard.nodes.forEach((node) => {
              ...
              const parentId =
                hasCopiedParent && node.parent ? nodeIdMap.get(node.parent) ?? null : null
              newNodes[newId] = {
                ...node,
                id: newId,
                parent: parentId,
            ...
            if (pastedIds.size === 1) {
              ...
              if (newNodes[id])
                ...
                set({ selText: id, modeText: 'Double-click text to edit inline' })
            }
          ...
          commitWorkspaceSnapshot: (action, before) => {
            ...
            set((state) => ({
              historyPast: [
                ...state.historyPast.slice(-(MAX_UNDO - 1)),
                cloneWorkspaceSnapshot(before),
              ],
              historyFuture: [],
              actionHistory: [
                ...state.actionHistory.slice(-(MAX_ACTION_HISTORY - 1)),
